...
class Excel_Filelist:
    ...
    def import_dictionary(self):
        ...
        try:
            ...
            filekey = [f for f in all_screenshots if os.path.isfile(screenshots_folder + '/' + f)]
            timestamps = [excel_fl.get_file_creation_date(t) for t in all_screenshots]
            dictionary = dict(zip(filekey, timestamps))
    ...
    def print_dictionary(self, dictionary):
        """ max_width formats all printed out statements to be a neat spaced out columns.
                ...
                ---         ---
        """
        max_width = max(len(str(f)) for f in dictionary.keys())
        print(*[f"{f:<{max_width}}\t{t}" for f, t in dictionary.items()], sep="\n")
...
if __name__ == '__main__':
    ...
    image_dir = r"C:\Users\Audrey\OneDrive\Pictures\random_images"
